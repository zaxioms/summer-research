...
def run_colmap(args):
    ...
    if os.path.exists(db):
        os.remove(db)
    do_system(f"colmap feature_extractor --ImageReader.camera_model OPENCV --SiftExtraction.estimate_affine_shape=true --SiftExtraction.domain_size_pooling=true --ImageReader.single_camera 1 --database_path {db} --image_path {images} --ImageReader.mask_path {masks}")
...
if __name__ == "__main__":
    ...
    with open(os.path.join(TEXT_FOLDER, "images.txt"), "r") as f:
        ...
        out = {
            ...
            "frames": [],
            "tot_fr": len(glob.glob('%s/*.jpg'%args.images)),
        }
    ...
    # load poitns3d
    manager= pycolmap.SceneManager('%s/colmap_sparse/0'%root_dir)
    manager.load_cameras()
    manager.load_points3D()
    manager.filter_points3D(min_track_len=5)
    points3d = manager.get_filtered_points3D()
    totp = points3d.mean(0)
    print(totp) 
    for f in out["frames"]:
